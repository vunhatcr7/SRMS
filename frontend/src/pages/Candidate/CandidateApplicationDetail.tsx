          ...
          <div className="grid grid-cols-5 gap-2">
            {Array.from({ length: 5 }).map((_, idx) => (
              <div key={idx} className="text-center">
                <Skeleton className="h-2 w-full rounded-full mb-2 mx-auto" />
    ...
    <div className="mx-auto max-w-4xl space-y-5">
      ...
      <div className={`rounded-lg border p-6 ${isDark ? 'border-navy-700 bg-navy-800' : 'border-slate-200 bg-white'}`}>
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <h1 className={`mt-1 text-2xl font-bold ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>{application.job.title}</h1>
      ...
      <div className={`rounded-lg border p-5 ${isDark ? 'border-navy-700 bg-navy-800' : 'border-slate-200 bg-white'}`}>
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <div>
            <h2 className={`text-base font-bold ${isDark ? 'text-slate-100' : 'text-slate-900'}`}>Recruitment progress</h2>
            <p className={`mt-1 text-xs ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>Current stage: <span className="ml-1"><StatusBadge stage={application.stage} /></span></p>
          </div>
            ...
            {stages.map((stage, index) => {
              ...
              return (
                <div key={stage} className="text-center">
                  <div className={`mx-auto h-2 w-full rounded-full mb-2 ${complete ? 'bg-brand' : isDark ? 'bg-navy-700' : 'bg-slate-200'}`} />
                  <span className={`text-[11px] font-medium ${complete ? (isDark ? 'text-slate-200' : 'text-slate-900') : (isDark ? 'text-slate-500' : 'text-slate-500')}`}>{getStageLabel(stage)}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <section className={`rounded-lg border p-5 ${isDark ? 'border-navy-700 bg-navy-800' : 'border-slate-200 bg-white'}`}>
          ...
          <p className={`mt-3 text-sm leading-relaxed whitespace-pre-line ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{application.job.description}</p>
        </section>
        <section className={`rounded-lg border p-5 ${isDark ? 'border-navy-700 bg-navy-800' : 'border-slate-200 bg-white'}`}>
          ...
          <p className={`mt-3 text-sm leading-relaxed whitespace-pre-line ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{application.job.requirements}</p>
        </section>
      </div>
    </div>
  );

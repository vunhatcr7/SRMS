          ...
          <article key={application.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 className="font-bold text-slate-900">{application.job.title}</h2>
                  <p className="mt-1 text-sm text-slate-600">
                    {application.job.company?.name || 'Company'} · {application.job.location}
                  </p>
                  <p className="mt-2 text-xs text-slate-500">Applied {formatDate(application.createdAt)}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className={`rounded-full border px-3 py-1 text-xs font-semibold ${getStageColor(application.stage)}`}>
                ...
                {typeof application.matchingScore === 'number' && (
                  <span className={`text-sm font-bold ${getScoreColor(application.matchingScore)}`}>
                    {Math.round(application.matchingScore)}% match
                  </span>
                )}
              </div>
            </div>
            ...
            <div className="mt-5 flex items-center justify-between gap-2 border-t border-slate-100 pt-3">
              ...
              {application.stage === 'INTERVIEW' && (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 rounded-lg bg-indigo-50 border border-indigo-200 px-3 py-1.5 text-xs font-bold text-indigo-700 hover:bg-indigo-100 transition"
                >
